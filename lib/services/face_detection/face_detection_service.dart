...
class FaceDetection extends AiModel {
  ...
  Map<String, dynamic>? predict(image_lib.Image image) {
    ...
    for (var detection in detections) {
      ...
      if (score > threshold) {
        bbox = Rect.fromLTRB(
          inputImage.width * detection.xMin-20,
          inputImage.height * detection.yMin-20,
          inputImage.width * detection.width-12,
          inputImage.height * detection.height-22,
        );
    ...
    return rectFaces[0];//rectFaces[0]['bbox'].left top right bottom
  }
  /*
  Map<String, dynamic>? predict(image_lib.Image image) {
    if (interpreter == null) {
...
Map<String, dynamic>? runFaceDetector(Map<String, dynamic> params) {
  //final faceDetection = FaceDetection(interpreter: Interpreter.fromAddress(params['detectorAddress']));
  //final a = locator<FaceDetection>();
  //final b = locator<ModelInferenceService>();
